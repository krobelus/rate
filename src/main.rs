...
use crate::{
    ...
    output::{solution, value, Timer , RuntimeResult},
    parser::{ParsingInfo , Parser , SyntaxFormat , BinaryMode} ,
    proof::{Proof} ,
};
...
fn run() -> RuntimeResult<i32> {
    ...
    .arg(Arg::with_name("RAT_FIRST_PIVOT").short("1").long("rat-first-pivot")
         .help("Assume that the RAT pivot is the first literal in a clause"))
    .arg(Arg::with_name("RAT_ANY_PIVOT").long("rat-any-pivot")
    ...
    .arg(Arg::with_name("RESPECT_UNIT_DELETIONS").long("respect-unit-deletions")
         .help("Respect deletion of unit clauses: specified check (default)"))

    .arg(Arg::with_name("DRAT_SYSTEM").long("drat")
         .help("Check proof with respect to the DRAT proof system (default)"))
    .arg(Arg::with_name("DPR_SYSTEM").short("p").long("dpr")
        .help("Check proof with respect to the DPR proof system"))
    .arg(Arg::with_name("DSR_SYSTEM").short("s").long("dsr")
        .help("Check proof with respect to the DSR proof system"))

    .arg(Arg::with_name("TEXT_FORMAT").short("t").long("text")
        .help("Assume the proof is provided in text format"))
    .arg(Arg::with_name("BINARY_FORMAT").short("b").long("binary")
        .help("Assume the proof is provided in binary format"))
    .arg(Arg::with_name("DRAT_TRIM_FORMAT").long("heuristic-binary-detect")
        .help("Detect text/binary with the drat-trim heuristic (default)"))
    .arg(Arg::with_name("PREFIX_FORMAT").short("x").long("prefix-binary-detect")
...
fn parse_files(config : &Config) -> RuntimeResult<Proof> {
    ...
    {
        let mut parser = Parser::new(parse_info , &config.formula_filename , SyntaxFormat::Dimacs , BinaryMode::Text)? ;
        parse_info = parser.parse()? ;
    }
    {
        let mut parser = Parser::new(parse_info , &config.proof_filename , SyntaxFormat::from(config.proof_format) , config.binary_mode)? ;
        parse_info = parser.parse()? ;
    }
